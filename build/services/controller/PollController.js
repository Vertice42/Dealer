...
class PollController {
     ...
     * @param Buttons
     */
    async StartDistribuition(Buttons) {
        ...
        let Bettings = await new dbPollManager_1.dbPollMager(this.StreamerID).getAllBettings();
     ...
     * @param BetAmount
     */
    async AddBet(TwitchUserID, BetID, BetAmount) {
        let AccountData = dbStreamerManager_1.dbStreamerManager.getAccountData(this.StreamerID);
        let Wallet = await new dbWalletManager_1.dbWalletManeger(this.StreamerID, TwitchUserID).getWallet();
        ...
        if (UpdateRes[0] == 0)
            await AccountData.dbCurrentBettings.create({
     ...
     * @param Buttons
     */
    async UpdateButtonsOfPoll(Buttons) {
    ...
    async dbUpdatePollStatus() {
        ...
        if (AccountData.CurrentPollID !== NewCurrentPollID &&
            ...
            await dbUtil_1.RenameTable(AccountData.dbStreamer, AccountData.CurrentPollID, NewCurrentPollID);
            await dbUtil_1.RenameTable(AccountData.dbStreamer, AccountData.CurrentBettingsID, NewCurrentBettingsID);
    ...
    async getCurrentPoll() {
        let AccountData = dbStreamerManager_1.dbStreamerManager.getAccountData(this.StreamerID);
        if (!AccountData)
            AccountData = await dbLoading_1.Loading.StreamerDatabase(this.StreamerID);
        let Buttons = [];
        let Bets;
        let db_pollMager = new dbPollManager_1.dbPollMager(this.StreamerID);
