...
export async function getMaxIDOfTable(table: typeof Model) {
    return Object.values((await table.sequelize.query(`SELECT MAX(ID) FROM ${table.tableName}`))[0][0])[0]
}
...
export async function DeleteTable(db: typeof Model) {
    return db.sequelize.query("DROP TABLE " + db.name);
}
