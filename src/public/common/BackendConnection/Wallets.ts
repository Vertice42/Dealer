...
export async function GetWallet(StreamerID: string, TwitchUserID: string) {
  return fetch(ServerConfigs.URL + getWalletRoute(StreamerID, TwitchUserID), {
    method: "GET"
  }).then((result) => {
...
export async function GetWallets(StreamerID: string, TwitchUserID = '*') {
  return fetch(ServerConfigs.URL + getWalletsRoute(StreamerID, TwitchUserID), {
    method: "GET"
  }).then((result) => {
    if (result.ok) return result.json();
    else return reject(result);
...
export async function ManagerWallets(WalletManagerRequest: WalletManagerRequest) {
  ...
  return fetch(ServerConfigs.URL + WalletManager, {
    method: "POST",
    headers: H,
